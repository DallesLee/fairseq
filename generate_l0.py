...
def _main(cfg: DictConfig, output_file):
    ...
    for num_to_unmask in list_of_nums:
        ...
        score = eval_bleu_score(
            cfg, saved_cfg, models, gates, lms, use_cuda, num_to_unmask
        )
...
def eval_bleu_score(
    cfg: DictConfig, saved_cfg, models, gates, lms, use_cuda, num_of_heads
):
    # Load dataset splits
    task = tasks.setup_task(cfg.task)
    ...
    head_mask = convert_gate_to_mask(gates, num_of_heads)
    head_mask = utils.move_to_cuda(head_mask) if use_cuda else head_mask
